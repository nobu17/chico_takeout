...
var inputs = []input{
	{name: "normal case",
		args:             args{name: "test1", priority: 1},
		want:             args{name: "test1", priority: 1},
		hasValidationErr: false,
	},
	{name: "normal case:name edge case(15char)",
		args:             args{name: "あいうえおかきくけこたちつてと", priority: 1},
		want:             args{name: "あいうえおかきくけこたちつてと", priority: 1},
		hasValidationErr: false,
	},
	{name: "error case:name empty",
		args:             args{name: "", priority: 1},
		hasValidationErr: true,
	},
	{name: "error case:name is over limit(16) 2byte",
		args:             args{name: "あいうえおかきくけこたちつてとな", priority: 1},
		hasValidationErr: true,
	},
	{name: "error case:name is over limit(16) 1byte",
		args:             args{name: "1234567890123455", priority: 1},
		hasValidationErr: true,
	},
...
func TestNewItemKind(t *testing.T) {
	for _, tt := range inputs {
		got, err := item.NewItemKind(tt.args.name, tt.args.priority)
		if err != nil {
...
func TestSetItemKind(t *testing.T) {
	for _, tt := range inputs {
		got, _ := item.NewItemKind("test", 4)
		err := got.Set(tt.args.name, tt.args.priority)
		if err != nil {
			var vErr *common.ValidationError
			if errors.As(err, &vErr) {
				if tt.hasValidationErr {
